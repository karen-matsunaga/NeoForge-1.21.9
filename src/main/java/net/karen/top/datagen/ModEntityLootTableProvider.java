    ...
    public void generate() {
        ...
        add(ModEntities.GECKO.get(),
            LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F))
                     .add(LootItem.lootTableItem(ModItems.GROWTH.get())
        ...
        add(ModEntities.MINER_BOW_ARROW_ENTITY.get(), LootTable.lootTable());
        // ** CUSTOM Sittable Block Model **
        add(ModEntities.CHAIR_ENTITY.get(), LootTable.lootTable());
        // ** CUSTOM Boat and Chest Boat **
        add(ModEntities.MOD_BOAT.get(), LootTable.lootTable());
        add(ModEntities.MOD_CHEST_BOAT.get(), LootTable.lootTable());
    }

    @Override
    protected @NotNull Stream<EntityType<?>> getKnownEntityTypes() {
        return BuiltInRegistries.ENTITY_TYPE.stream().filter(type -> BuiltInRegistries.ENTITY_TYPE
                                                                                                   .getKey(type).getNamespace()
                                                                                                   .equals(Top.MOD_ID));
    }
}