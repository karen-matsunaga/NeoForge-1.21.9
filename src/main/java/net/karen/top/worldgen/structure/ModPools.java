...
public class ModPools {
    ...
    public static final ResourceKey<StructureTemplatePool> STORAGE_PLATFORM_SIDE =
           create("storage_platform/side_pool");

    public static final ResourceKey<StructureTemplatePool> STORAGE_PLATFORM_START =
           create("storage_platform/start_pool");

    // CUSTOM METHOD - Register all custom template pools
    private static ResourceKey<StructureTemplatePool> create(String name) {
        return Utils.rKey(Registries.TEMPLATE_POOL, Utils.topPath(name));
    }
    ...
    public static void bootstrap(BootstrapContext<StructureTemplatePool> context) {
        ...
        context.register(KAUPEN_HOUSE,
                         new StructureTemplatePool(empty,
                                                   List.of(Pair.of(StructurePoolElement.single(Top.MOD_ID + ":kaupen_house"),
                                                                   1)),
        ...
        context.register(STORAGE_PLATFORM_SIDE,
                         new StructureTemplatePool(empty,
                                                   List.of(Pair.of(StructurePoolElement.single(Top.MOD_ID +
                                                                                               ":diamond_storage_platform"),
                                                                   2),
                                                           Pair.of(StructurePoolElement.single(Top.MOD_ID +
                                                                                               ":gold_storage_platform"),
                                                                   3),
                                                           Pair.of(StructurePoolElement.single(Top.MOD_ID +
                                                                                               ":iron_storage_platform"),
                                                                   5)),
                                                   StructureTemplatePool.Projection.RIGID));
        ...
        context.register(STORAGE_PLATFORM_START,
                         new StructureTemplatePool(empty,
                                                   List.of(Pair.of(StructurePoolElement.single(Top.MOD_ID + ":storage_platform"),
                                                                   1)),
                                                   StructureTemplatePool.Projection.RIGID));
    }
}