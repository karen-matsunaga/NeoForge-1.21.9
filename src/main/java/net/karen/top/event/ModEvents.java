...
import net.karen.top.enchantment.custom.LightningStrikerEnchantmentEffect;
...
import static net.karen.top.enchantment.custom.AutoSmeltEnchantmentEffect.autoSmeltEnch;
import static net.karen.top.enchantment.custom.MoreOresEnchantmentEffect.moreOresEnch;
import static net.karen.top.enchantment.custom.RainbowEnchantmentEffect.rainbowEnch;
import static net.karen.top.item.custom.SwordEffectItem.*;
    ...
    */
    ...
    public static void addCustomTrades(VillagerTradesEvent event) {
        ...
        if (event.getType() == ModVillagers.SOUND_MASTER.getKey()) {
            ...
            trades.get(2).add((_, _) -> new MerchantOffer(
                              new ItemCost(ModItems.SPECIAL_BOTTLE, 1)
            ...
            trades.get(3).add((_, _) -> new MerchantOffer(
                              // Item Cost
                              new ItemCost(ModItems.SPECIAL_BOTTLE.get(), 1)
                                          .withComponents(value ->
    ...
    public static void onTopElevatorKeyInput(InputEvent.Key event) {
        ...
        if (player != null) { // Checks if the player is over the elevator
            ...
            if (player.level().getBlockState(pos).getBlock() == ModBlocks.TOP_ELEVATOR.get()) { // Detects JUMP or SHIFT
                if (InputConstants.isKeyDown(mc.getWindow(), GLFW.GLFW_KEY_SPACE)) {
                    ClientPacketDistributor.sendToServer(new TopElevatorPacketPayload(true));
                }
                if (player.isShiftKeyDown()) {
                    ClientPacketDistributor.sendToServer(new TopElevatorPacketPayload(false));
                }
            }
        }
    }
    ...
    public static void onSpecialBottleMouseButton(InputEvent.MouseButton.Post event) {
        ...
        if (player != null && clientLevel != null) { // Pressed LEFT click + SHIFT
            ...
            if (mc.options.keyAttack.isDown() && item instanceof SpecialBottleItem self) {
                ClientPacketDistributor.sendToServer(new SpecialBottlePacketPayload(
                                                     SpecialBottlePacketPayload.SpecialBottleEnum.STORED,
    ...
    public static void onSpecialBottleKeyInput(InputEvent.Key event) {
        ...
        if (mc.player != null && mc.level != null) {
            if (ModKeyMapping.SPECIAL_BOTTLE_STORED_KEY.get().consumeClick()) { // Pressed SHIFT + N
                ClientPacketDistributor.sendToServer(new SpecialBottlePacketPayload(
                                                     SpecialBottlePacketPayload.SpecialBottleEnum.STORED,
                                                     shift ? 100 : 10));
            }
            if (ModKeyMapping.SPECIAL_BOTTLE_RESTORED_KEY.get().consumeClick()) { // Pressed SHIFT + B
                ClientPacketDistributor.sendToServer(new SpecialBottlePacketPayload(
                                                     SpecialBottlePacketPayload.SpecialBottleEnum.RESTORED,
                                                     shift ? 100 : 10));
            }
        }
    }
